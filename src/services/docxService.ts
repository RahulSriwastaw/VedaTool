...
function isChemicalFormula(latex: string): boolean {
  // Detect standard chemical patterns, including arrows and heat/pressure above arrows
  ...
  if (
    ...
    latex.includes("\\to") ||
    latex.includes("longrightarrow")
  )
    return true;

  // Check for common chemical symbols with state
  if (
    /[A-Z][a-z]?\d*\s*\([a-z]\)/.test(latex) &&
    (latex.includes("+") || latex.includes("->") || latex.includes("\\to"))
  )
    return true;
...
function parseChemistryToTextRuns(latex: string, isBold: boolean): any[] {
  ...
  // Improved processing for chemical formulas
  let processed = latex
    .replace(/\\xrightarrow\[([^\]]+)\]\{([^}]+)\}/g, " ⟶ [$1, $2] ⟶ ") // Handles arrow with both above/below
  ...
  // Replace common labels that might be in LaTeX
  processed = processed.replace(/\\text\{([^}]+)\}/g, "$1");

  // Tokenize manually for better control
  const tokens = processed.split(/([_^\\]|[ \t\n]+)/g).filter(Boolean);
  ...
  while (i < tokens.length) {
      ...
      if (i < tokens.length) {
        ...
        runs.push(
          new TextRun({
            ...
            subScript: isSub,
            superScript: !isSub,
            size: 20, // Slightly smaller for sub/super
            font: "Arial",
            bold: isBold,
            noProof: true,
          }),
    ...
    // Skip structural curly braces that can remain
    if (token === "{" || token === "}") {
      i++;
      continue;
    }

    // Keep spaces, word tokens, and other characters (including Hindi/Unicode, symbols, operators etc.)
    runs.push(
      new TextRun({
        text: token,
        size: 22,
...
function ensureNonEmptyNodeList(nodes: any[]): any[] {
    ...
    if (char === "\\") {
      ...
      if (remainder.startsWith("frac") || remainder.startsWith("binom")) {
        ...
        if (MathFraction) {
          nodes.push(
            new MathFraction({
              numerator: ensureNonEmptyNodeList(parseLatex(num)),
              denominator: ensureNonEmptyNodeList(parseLatex(den)),
            }),
          );
        } else {
          nodes.push(
            new MathRun(isBinom ? `(${num} over ${den})` : `(${num}/${den})`),
          );
        }
        continue;
      }
      ...
      if (remainder.startsWith("sqrt")) {
        ...
        if (MathRadical) {
          nodes.push(
            new MathRadical({
              degree: optArg ? ensureNonEmptyNodeList(parseLatex(optArg)) : undefined,
              children: ensureNonEmptyNodeList(parseLatex(inner)),
            }),
          );
        } else {
      ...
      if (naryMatch) {
        ...
        if (
          ...
          let limitLocation: any =
            cmd.includes("int") || cmd.includes("oint") ? "subSup" : "undOvr";
        ...
        if (MathSubSuperScript) {
          nodes.push(
            new MathSubSuperScript({
              children: [base],
              subScript: ensureNonEmptyNodeList(parseLatex(subArgText!)),
              superScript: ensureNonEmptyNodeList(parseLatex(supArgText!)),
            }),
          );
        } else {
          nodes.push(base);
        ...
        if (isSup) {
          if (MathSuperScript) {
            nodes.push(
              new MathSuperScript({
                children: [base],
                superScript: ensureNonEmptyNodeList(parseLatex(argContent)),
              }),
            );
          } else {
            nodes.push(base);
          ...
          if (MathSubScript) {
            nodes.push(
              new MathSubScript({
                children: [base],
                subScript: ensureNonEmptyNodeList(parseLatex(argContent)),
              }),
            );
          } else {
            nodes.push(base);
...
function replaceLatexSymbolsInPlainStr(text: string): string {
  let replaced = text.replace(/\\([a-zA-Z]+)/g, (match, cmd) => {
    if (LATEX_SYMBOLS[cmd]) {
      return LATEX_SYMBOLS[cmd];
    }
    if (MATH_FUNCTIONS.includes(cmd)) {
      return cmd; // e.g. \tan -> tan, \sin -> sin
    }
    return match;
  });
  // clean up escaped braces if they exist in normal text e.g. \{ and \}
  replaced = replaced.replace(/\\\{/g, "{").replace(/\\\}/g, "}");
  return replaced;
}

function parseTextWithSubSuperScripts(
  text: string,
  isBold: boolean,
  style?: any,
): TextRun[] {
        ...
        if (scriptText) {
          runs.push(
            new TextRun({
              ...
              superScript: !isSub,
              font: "Arial",
              size: style?.fontSize ? style.fontSize * 2 : 20, // slightly smaller for sub/super
              bold: isBold,
              color: style?.color,
      ...
      if (normalText) {
        runs.push(
          new TextRun({
            text: normalText,
            font: "Arial",
            size: style?.fontSize ? style.fontSize * 2 : 22,
            bold: isBold,
            color: style?.color,
            italics: style?.isItalic, // Note: TextRun property is 'italics', not 'isItalic'
            noProof: true,
          }),
        );
      }
...
function parseLineToChildren(
  ...
  const flattenChildren = (arr: any[]): any[] => {
    const result: any[] = [];
  ...
  const rawChildren = parts.map((part) => {
    // CASE 1: Math Block
    if (part.startsWith("$$") && part.endsWith("$$")) {
      ...
      if (DocxMath) {
        try {
          return new DocxMath({ children: ensureNonEmptyNodeList(parseLatex(latex)) });
        } catch (e) {
          console.error("Error parsing LaTeX:", latex, e);
          // Fallback to text if math fails
          const clean = replaceLatexSymbolsInPlainStr(latex);
          return parseTextWithSubSuperScripts(clean, forceBold);
        }
      } else {
        const clean = replaceLatexSymbolsInPlainStr(latex);
        return parseTextWithSubSuperScripts(clean, forceBold);
    ...
    // CASE 2: Text Block (May contain **Bold**)
      ...
      return boldParts.map((subPart) => {
        ...
        const cleanedText = replaceLatexSymbolsInPlainStr(cleanText);
        return parseTextWithSubSuperScripts(cleanedText, isBold, style);
      });
    }
  });

  return flattenChildren(rawChildren);
}
...
function stripHtmlTagsPreservingMath(html: string): string {
  if (!html) return "";

  // 1. Extract and safeguard all math blocks (display math, inline math and LaTeX commands)
  ...
  // Replace each math block with a unique placeholder
  for (const regex of mathDelimiters) {
  ...
  // 2. Clean HTML elements while protecting formatting blocks
  ...
  // Decode standard HTML entities and whitespace
  ...
  // 3. Restore preserved math blocks exactly as they were
  for (let i = 0; i < placeholders.length; i++) {
    text = text.replace(`[[MATH_PLACEHOLDER_${i}]]`, placeholders[i]);
  }

  // 4. Force strip any invalid XML control characters that crash MS Word
...
export interface DocxExportOptions {
  ...
  pageSize?: "A4" | "Letter";
  fontFamily?: string;
  marginType?: "narrow" | "normal" | "wide";
  showNameRollNoFields?: boolean;
}
...
export const generateDocx = async (
  ...
  // Prepend headers & details in docxOptions if provided
  if (docxOptions?.pageHeaderTitle) {
    docChildren.push(
      new Paragraph({
        children: [
          new TextRun({
            text: docxOptions.pageHeaderTitle.toUpperCase(),
            bold: true,
            size: 28, // 14pt
            font: docxOptions.fontFamily || "Calibri",
          }),
        ],
        alignment: AlignmentType.CENTER,
  ...
  if (docxOptions?.pageHeaderSubtitle) {
    docChildren.push(
      new Paragraph({
        children: [
          new TextRun({
            text: docxOptions.pageHeaderSubtitle,
            italics: true,
            size: 20, // 10pt
            font: docxOptions.fontFamily || "Calibri",
          }),
  ...
  if (docxOptions?.showNameRollNoFields) {
    docChildren.push(
      new Paragraph({
        children: [
          new TextRun({
            text: "Student Name: _______________________          Roll No: _______________",
            bold: true,
            size: 18, // 9pt
            font: docxOptions.fontFamily || "Calibri",
          }),
  ...
  const flushOptions = () => {
    ...
    if (optionArrangement === OptionArrangement.VERTICAL) {
      for (const opt of optionBuffer) {
        // Replace first space after label (like `(a) `, `a. ` or `A) `) with a tab for proper alignment
        const tabbedOpt = opt.replace(
          /^(\([a-zA-Z0-9]\)|[a-zA-Z0-9][\.\)]|[A-Z][\.\)])\s+/,
          "$1\t",
        );
        docChildren.push(
          new Paragraph({
            children: parseLineToChildren(tabbedOpt) as any[],
            indent: { left: 720, hanging: 360 }, // Align to 0.5 inches left, label starts at 0.25 inches
      ...
      optionBuffer.forEach((opt, idx) => {
        ...
        if (idx > 0) {
          children.push(new TextRun({ text: "\t" })); // Tab spacing between consecutive options
        }
        children.push(...parseLineToChildren(tabbedOpt));
      });
      docChildren.push(
        new Paragraph({
          children: children,
      ...
      for (let i = 0; i < optionBuffer.length; i += 2) {
        ...
        const opt1 = pair[0].replace(
          /^(\([a-zA-Z0-9]\)|[a-zA-Z0-9][\.\)]|[A-Z][\.\)])\s+/,
          "$1\t",
        );
        children.push(...parseLineToChildren(opt1));

        if (pair.length > 1) {
          children.push(new TextRun({ text: "\t" }));
          const opt2 = pair[1].replace(
            /^(\([a-zA-Z0-9]\)|[a-zA-Z0-9][\.\)]|[A-Z][\.\)])\s+/,
            "$1\t",
          );
          children.push(...parseLineToChildren(opt2));
        }
        ...
        docChildren.push(
          new Paragraph({
            ...
            tabStops: [
              { type: TabStopType.LEFT, position: 4000 },
              { type: TabStopType.LEFT, position: 4360 },
            ],
            indent: { left: 720, hanging: 360 },
            spacing: { before: 40, after: 40, line: 240 },
            alignment: AlignmentType.LEFT,
  ...
  for (const element of elements) {
    const isComplexTable =
      element.type === "table" &&
      element.content &&
      isTableTooComplex(element.content.split("\n"));
      ...
      if (isComplexTable) {
        docChildren.push(
          new Paragraph({
            children: [
              new TextRun({
                text: "(Table rendered as image due to complexity)",
                size: 16,
                italics: true,
                color: "666666",
              }),
    ...
    // Clean modern HTML tags and protect LaTeX math markup perfectly
    const content = stripHtmlTagsPreservingMath(element.content || "");
    ...
    for (let i = 0; i < rawLines.length; i++) {
      ...
      if (!line) {
        if (currentLineBuffer)
          lines.push(currentLineBuffer.replace(/ \[\[MATH_BR\]\] /g, "\n"));
        currentLineBuffer = "";
        continue;
      }
      ...
      const isMetadata =
        /^(Subject|Time|Max\.?\s*Marks|Marks|Class|Date|Roll\s*No|Duration)\s*[:\-]/i.test(
          cleanLineText,
        );
      const isInstruction =
        /^(Note|Instructions?|General\s*Instructions?)\s*[:\-]/i.test(
          cleanLineText,
        );
      const isSeparator =
        /^(\(OR\)|OR|अथवा|Athava|[\/]\s*OR|OR\s*[\/]|\s)+$/i.test(
          cleanLineText.replace(/[^a-zA-Z\u0900-\u097F\/]/g, "").trim(),
        );
      const isFullEquation = line.includes("$$");
      const isMainQuestion =
        /^#\s/i.test(cleanLineText) ||
        /^(Q\.?\s?\d+|Prashn\s?\d+|Question\s?\d+|प्रश्न\s?\d+|\d+\.|[\(\[]\d+[\)\]]|\d+[\)])\s/i.test(
          cleanLineText,
        );
      const isSubQuestion =
        /^(\([ivxIVX]+\)|[ivxIVX]+\.|[ivxIVX]+[\)]|[\(\[]\w+[\)\]])\s/i.test(
          cleanLineText,
        );
      const isOption =
        /^(\([a-zA-Z0-9]\)|[a-zA-Z0-9][\.\)]|[A-Z][\.\)])(\s|$)/.test(
          cleanLineText,
        );
      const isTableRow =
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      let spacing = { before: 80, after: 80, line: 276 }; // Default paragraph spacing
      ...
      if (isSubQuestion) {
        ...
        spacing = { before: 180, after: 60, line: 276 }; // 180 before (9pt), 60 after (3pt)
      }
  ...
  const marginSize = docxOptions?.marginType === "normal" ? 1440 : docxOptions?.marginType === "wide" ? 2160 : 720;
  const pageSpecs = docxOptions?.pageSize === "Letter" 
    ? { width: 12240, height: 15840 } 
    : { width: 11906, height: 16838 }; // Default A4
